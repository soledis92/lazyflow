...
class OpCompressedUserCropArray(OpUnmanagedCompressedCache):
    ...
    eraser = InputSlot()
    deleteCrop = InputSlot(optional = True)
    blockShape = InputSlot() # If the blockshape is changed after crops have been stored, all cache data is lost.
    ...
    Projection2D = OutputSlot() # A somewhat magic output that returns a projection of all
                                ...
                                # - Exactly 0.0 means "no crops under this pixel"
                                # - 1/256.0 means "crops in the first slice"
                                # - ...
                                # - 1.0 means "last slice"
                                # The output is suitable for display in a colortable.
    
    def __init__(self, *args, **kwargs):
        super(OpCompressedUserCropArray, self).__init__( *args, **kwargs )
        self._blockshape = None
        self._crop_to_purge = 0
    ...
    def setupOutputs(self):
        ...
        super( OpCompressedUserCropArray, self ).setupOutputs()
        ...
        if self._blockshape is None:
            ...
            if len(nonzero_blocks) > 0:
                raise RuntimeError( "You are not permitted to reconfigure the cropping operator after you've already stored crops in it." )
        ...
        # Are we being told to delete a crop?
        if self.deleteCrop.ready():
            new_purge_crop = self.deleteCrop.value
            if self._crop_to_purge != new_purge_crop:
                self._crop_to_purge = new_purge_crop
                if self._crop_to_purge > 0:
                    self._purge_crop( self._crop_to_purge )
    
    def _purge_crop(self, crop_to_purge):
        """
        Scan through all croped pixels.
        (1) Clear all pixels of the given value (set to 0)
        (2) Decrement all crops above that value so the set of stored crops is consecutive
        """
        changed_block_rois = []
        ...
        for block_roi in stored_block_rois:
            ...
            matching_crop_coords = numpy.nonzero( block == crop_to_purge )
            coords_to_decrement = block > crop_to_purge

            # Change the data
            block[matching_crop_coords] = 0
            block = numpy.where( coords_to_decrement, block-1, block )
            
            # Update cache with the new data (only if something really changed)
            if len(matching_crop_coords[0]) > 0 or len(coords_to_decrement[0]) > 0:
                super( OpCompressedUserCropArray, self )._setInSlotInput( self.Input, (), SubRegion( self.Output, *block_roi ), block, store_zero_blocks=False )
    ...
    def execute(self, slot, subindex, roi, destination):
        if slot == self.Output:
            ...
            return super( OpCompressedUserCropArray, self ).execute( slot, subindex, roi, destination )
    ...
    def _executeProjection2D(self, roi, destination):
        ...
        # Proceed with the projection, working blockwise to avoid unecessary work in uncroped blocks
        ...
        for block_start in block_starts:
            if block_start not in self._cacheFiles:
                # No crop data in this block.  Move on.
                continue
            ...
            def calc_color_value(slice_index):
                ...
                #           crops in the "back" slices would appear occluded.  But the 
    ...
    def _setInSlotInput(self, slot, subindex, roi, new_pixels):
        """
        Since this is a crop array, inserting pixels has a special meaning:
        ...
        super( OpCompressedUserCropArray, self )._setInSlotInput( slot, subindex, roi, cleaned_data, store_zero_blocks=False )
    ...
    def ingestData(self, slot):
        ...
        Returns: the max crop found in the slot.
        """
        ...
        for block_start in block_starts:
            ...
            max_crop = max( max_crop, cleaned_block_data.max() )
